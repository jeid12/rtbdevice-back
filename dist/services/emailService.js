...
exports.sendAutomationReport = exports.sendDeviceAssignmentNotification = exports.sendOfflineDeviceAlert = exports.sendWarrantyExpiryAlert = exports.sendMaintenanceReminder = exports.sendOtpEmail = void 0;
 ...
 */
const sendMaintenanceReminder = async (devices, recipients) => {
 ...
 */
const sendWarrantyExpiryAlert = async (devices, recipients) => {
...
exports.sendWarrantyExpiryAlert = sendWarrantyExpiryAlert;
/**
 * Send offline device notifications
 */
const sendOfflineDeviceAlert = async (devices, recipients) => {
...
exports.sendOfflineDeviceAlert = sendOfflineDeviceAlert;
/**
 * Send device assignment notifications
 */
const sendDeviceAssignmentNotification = async (device, school, recipients) => {
...
exports.sendDeviceAssignmentNotification = sendDeviceAssignmentNotification;
/**
 * Send automation report
 */
const sendAutomationReport = async (reportData, recipients) => {
...
exports.sendAutomationReport = sendAutomationReport;
/**
 * Generate maintenance reminder template
 */
const generateMaintenanceReminderTemplate = (devices) => {
    const deviceList = devices.map(d => `- ${d.name_tag} (${d.model}) at ${d.school?.name || 'Unassigned'}`).join('\n');
    return {
 ...
 */
const generateWarrantyExpiryTemplate = (devices) => {
    const deviceList = devices.map(d => `- ${d.name_tag} (${d.model}) - Expires: ${d.warrantyExpiry?.toLocaleDateString()}`).join('\n');
    return {
 ...
 */
const generateOfflineDeviceTemplate = (devices) => {
    const deviceList = devices.map(d => `- ${d.name_tag} (${d.model}) - Last seen: ${d.lastSeenAt?.toLocaleDateString()}`).join('\n');
    return {
 ...
 */
const generateDeviceAssignmentTemplate = (device, school) => {
    return {
 ...
 */
const generateAutomationReportTemplate = (reportData) => {
    return {
